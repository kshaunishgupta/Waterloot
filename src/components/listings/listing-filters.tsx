...
import { CONDITIONS, DATE_FILTERS } from "@/lib/constants";
...
export function ListingFilters({ categories, currentFilters }: ListingFiltersProps) {
  const router = useRouter();
  const searchParams = useSearchParams();

  const [localMin, setLocalMin] = useState(currentFilters.min_price || "");
  const [localMax, setLocalMax] = useState(currentFilters.max_price || "");
  ...
  const applyPrice = useCallback(() => {
    const params = new URLSearchParams(searchParams.toString());
    if (localMin) params.set("min_price", localMin); else params.delete("min_price");
    if (localMax) params.set("max_price", localMax); else params.delete("max_price");
    params.delete("page");
    router.push(`/browse?${params.toString()}`, { scroll: false });
  ...
  // Separate housing from regular categories
  ...
  const housingSubcategories = housingParent
    ...
    : [];
  // Exclude housing parent + all subcategories from the main category list
  const housingIds = new Set([
    housingParent?.id,
    ...housingSubcategories.map((c) => c.id),
  ].filter(Boolean));
  const regularCategories = categories.filter((c) => !housingIds.has(c.id) && !c.parent_id);
  ...
  return (
    <aside className="space-y-6">
      ...
      {/* Category — multi-select rectangular boxes */}
